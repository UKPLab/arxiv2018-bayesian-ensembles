...
@author: Edwin Simpson, Melvin Laux
'''
import sys, logging
import numpy as np
...
from src.ibccdata import DataHandler
...
# Run the inference algorithm --------------------------------------------------------------------------------------    
    ...
    def run_inference(self):   
        ...
        while not converged and self.keeprunning:
            ...
            if np.any(self.E_t):
                self.post_alpha()
            self.lnPi = calc_q_pi(self.lnPi, self.alpha, self.nscores)
...
def calc_q_pi(lnPi, alpha, nscores):
