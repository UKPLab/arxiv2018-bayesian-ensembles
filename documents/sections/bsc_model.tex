\section{A Model for Bayesian Sequence Combination}\label{sec:model}
...
& \hspace{3.4cm} B^{(s)}(\bs t_{n,\tau}, J, d_{n,\tau-1}^{(s)})]). &&
\end{flalign}

% *** Check that this is right. Ideally, we could have a discriminative model here. If it's right add a comment about independence of ds from multiple sequence taggers. DO I already mention that the use of B means we can compensate
% for unreliable sequence taggers or soften labels from those that don't produce probabilities. Also could include
% labellers from other domains where theta is pretrained. ***
% Even if we took a discriminative approach, we still get dependencies between the taggers.
...
& \nonumber \\
& \prod_{j=1}^J p(\bs T_j | \bs\gamma_j)  \prod_{n=1}^N \prod_{\tau=1}^{L_n} p(\bs t_n | \bs T_{t_{n,\tau-1}}) \prod_{s=1}^S \bigg\{ p(\bs \theta^{(s)})  \nonumber \\
...
%TODO have we mentioned that the sequence taggers have their own \pi?
