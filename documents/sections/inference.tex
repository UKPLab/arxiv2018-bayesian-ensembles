...
%  randomly or to expectations with respect to the priors\;
 ...
 \nl Update $\ln q(A^{(k)})$ and $\mathbb{E}\left\ln A^{(k)},\forall k$ \;%given current $\bs c$, $r_{j,n,\tau}$\;
 \nl Update $\ln q(B^{(s)})$ and $\mathbb{E}\ln B^{(s)},\forall s$ \;%given current $\hat{\bs d}$, $r_{j,n,\tau}$\;
 \nl Update $\ln q(\bs T_j)$ and $\mathbb{E}\ln \bs T_j,\forall j$ \;%given current $s_{t_{j,n,\tau\!-\!1}, t_{\iota,n,\tau}}$\;
 }
\KwOut{ Label posteriors, $r_{n,\tau,j},\forall n,\forall \tau, \forall j$, %\mathbb{E}[\bs t]$, 
most probable sequence of labels, $\hat{\bs t}$ }
\;
