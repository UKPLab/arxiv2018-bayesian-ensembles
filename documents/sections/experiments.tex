...
the BiLSTM-LSTM-CRF of Lample et al.~\shortcite{lample2016neural} as a black-box sequence tagger, labeled \emph{BSC-seq+LSTM}.
...
0.79 & 0 & .1 & 0 \\ 
%HMM-crowd$\Arrow{0.2cm}$LSTMp & 81.3 & 68.6 & 74.4 & %76.1 & .9887 & 
%0.25 & 134 &  \\ 
...
% Leads to the question, what about IBCC with text features? --> does improvement of BSC-CM over IBCC come from text features? Would it improve more without the HMM? See IBCC+text
% Also leads to question, does the seq annotator model need the HMM? Maybe it would improve more over IBCC if it were scrapped? Maybe its performance advantages come from seq and features but not HMM? See IBCC-seq+text
...
%BSC-seq$\Arrow{0.2cm}$LSTMp & 80.3 & 74.6 & 77.3 & %78.6 & .9910 & 
%0.20 & 129 & .1, 1, 1 \\
BSC-seq$\Arrow{0.2cm}$LSTM & 80.2 & 75.3 & 77.7 & %79.6 & .9262 & 
11.0 & .1 & 1 & 1 & 
...
%0.54 & 0 & .1, 1, 1 \\
BSC-seq+LSTM & 
82.3 & \textbf{75.9} & \textbf{78.9} & 0.59 %79.6 & .9513 & % this requires 3 epochs then no changes to the model
%81.5 & \textbf{75.5} & \textbf{78.4} & 0.74 
& .1 & 1 & 1 & 
...
This likely reduces over-fitting resulting from the maximum likelihood step used to integrate the LSTM as a black-box sequence tagger.
...
Integrating an LSTM improves performance further in both datasets, and outperforms an LSTM trained on the output of HMM-crowd or BSC-seq.% to train an LSTM produces a small improvement, but is outperformed by BSC-seq+LSTM.
% Removed the data on no. invalid spans:
% 15, 81, 81, 92, 74, 68, 0.... BSC-MACE=91
% 0, 0, 80, 30, 54, 37, 0, 40, 0000000000000, BSC\Arrow{0.2cm}LSTM 80
...
\end{table*}
%To get a deeper understanding of the key methods, w
...
HMM-crowd$\Arrow{0.2cm}$LSTM & \textbf{78.7} & 59.0 & 67.5 &% 69.4 & .8642 &
 15.9  & \textbf{75.6} & 61.6 & 67.9 & % \textbf{76.4} & .8380 &
13.5 \\
BSC-seq$\Arrow{0.2cm}$LSTM & 74.3 & \textbf{62.8} & \textbf{68.1} & %69.7 & .8868 & 
...
account for uncertainty in the model parameters, hence can be used to choose data points 
that most rapidly reduce that uncertainty. 
...
We simulate active learning using \emph{uncertainty sampling}~\cite{settles2010active},
