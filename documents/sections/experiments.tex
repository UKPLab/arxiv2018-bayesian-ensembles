...
\end{table*}
%TODO put correct sentence counts in the above table. 
%TODO note that LSTM has purely discrete inputs/outputs. Do we need the probabilistic ones 
% to compare CEE? -- yes. Put the best CEE out of either proba or discrete.
...
\nprounddigits{1}
\npdecimalsign{.}
\begin{tabularx}{\textwidth}{| p{2.4cm} h X | X | X | X | P{0.3cm} | P{0.4cm} | P{0.3cm} ? X | X | X | X | P{0.4cm} | P{0.4cm} | P{0.3cm} ?}
...
%14.01 & 86 & .1, 1, .1 \\ 
%IBCC+text & \\ %TODO get the results for this. This isolates the benefits of the HMM transition when compared to BSC-CM (does BSC-CM benefit from text or from HMM? When tested without text but with HMM, it does worse than IBCC. So perhaps they are needed in combination).
%IBCC-seq+text & \\ % If IBCC+text is better than BSC-CM it makes sense to see if switching off the HMM also helps with seq...
\hline
...
\npnoround
\end{table*}
%TODO note discrete LSTM outputs. The alternative would be to switch the output layer to probabilities? This decreases f1 score though while improving CEE, 
%whereas our integration method increases it on tasks 1 and 2. I guess benefits would be clearer in a proper ensemble. I guess F1 score is the main support for integrated LSTM on task 1. On task 2, 
% it seems a little unnecessary unless you cannot add an output layer onto the black box.
