...
# Run the inference algorithm --------------------------------------------------------------------------------------    
    ...
    def run_inference(self):   
        ...
        while not converged and self.keeprunning:
            ...
            #update params
            #self.expec_lnkappa()
            self.lnkappa, self.nu = expec_lnkappa(self.E_t, self.nu0)
            #self.expec_lnPi()
            if np.any(self.E_t):
                self.post_Alpha()
            self.lnPi = expec_lnPi(self.lnPi, self.E_t, self.alpha, self.nscores)
...
# static functions
def expec_lnkappa(E_t, nu0):
    nu = nu0 + np.reshape(np.sum(E_t,0), nu0.shape)
    lnkappa = (psi(nu).T - psi(np.sum(nu, -1))).T
    return lnkappa, nu

def expec_lnPi(lnPi, E_t, alpha, nscores, posterior=True):
...
def expec_t(lnpCT, testidxs):
    ...
    pT = joint / norma
    return pT
    #self.E_t[testidxs, :] = pT  
    
def expec_t_trans(E_t):
    (n, k) = E_t.shape
